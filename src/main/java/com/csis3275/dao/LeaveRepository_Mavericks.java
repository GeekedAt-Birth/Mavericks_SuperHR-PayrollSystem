...
public interface LeaveRepository_Mavericks extends JpaRepository<LeaveApplications_Mavericks, Integer> {
	...
	@Query(value = "SELECT id, start_date, end_date, reason, leave_type, application_status, employee_Id FROM Leave_Applications_Mavericks WHERE application_status = 'pending'", nativeQuery = true)
	List<LeaveApplications_Mavericks> listPendingForms();

	@Query(value = "SELECT id, start_date, end_date, reason, leave_type, application_status, employee_Id FROM Leave_Applications_Mavericks WHERE application_status = 'approved'", nativeQuery = true)
	List<LeaveApplications_Mavericks> listAcceptedForms();

	@Query(value = "SELECT id, start_date, end_date, reason, leave_type, application_status, employee_Id FROM Leave_Applications_Mavericks WHERE application_status = 'rejected'", nativeQuery = true)
